...
class AnyCPTListingGutenbergBlock
{
    ...
    {   
        ...
        wp_register_script( $edit_block_script, plugin_dir_url(__FILE__) . 'build/index.js', ['wp-blocks', 'wp-element', 'wp-editor'], false, false );
    ...
    {   
        ...
        <div class="<?php echo isset($className) ? $className : ''; ?> acpt-main acpt-front-screen" >
            <div class="acpt-row">
                <?php if(!empty($posts)) {
                    foreach ($posts as $post) {
                        $post_id = $post->ID;
                        $post_thumbnail_url = wp_get_attachment_url( get_post_thumbnail_id( $post_id ) );
                        $featured_image_url = !empty($post_thumbnail_url) ? $post_thumbnail_url : $placeholder_image;
